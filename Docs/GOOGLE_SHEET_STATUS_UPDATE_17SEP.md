# Google Sheet update — Reloved Friday (17 Sep 2026)

**Live sheet:** https://docs.google.com/spreadsheets/d/1wDxhYYrWyyD6cPYNmKDkuqNYDFK6YZECvS5DG-ve3QI/edit?gid=1558858514

I cannot write to Google Sheets from Cursor (no Google Drive/Sheets API connected).  
**Local mirror updated:** `Docs/Reloved_Friday_F&F_Task_Sheet.csv` (and root copy).

## Fastest way to update the Google Sheet

1. Open the sheet tab `Reloved_Friday_F&F_Task_Sheet`
2. **File → Import → Upload** `Docs/Reloved_Friday_F&F_Task_Sheet.csv`
3. Choose **Replace current sheet** (or import as new tab then copy Status column)

Or paste Status manually using the table below (match **Task ID** in column A → set **Status**).

## Status values to set (from this conversation)

| Task ID | New Status | Notes |
|---|---|---|
| P0-17 | Done (code) | Borzo-only; Porter CTAs removed |
...
| P2-04 | Done (code) | Prepaid lock |
| P2-06 | Done (code) early | First-500 Reloved subsidy counter |

## Add these new rows (CEO 17 Sep meeting)

| Task ID | Priority | Category | Task | Status | Owner | Exact change |
|---|---|---|---|---|---|---|
