...
**Give an item** (`Give.tsx`) — multi-step form: photos → AI-assisted category/title/condition suggestions (relayed through the backend to an external photo-analysis server) → pickup details → legal consent (`LegalAccept`) → submit. Triggers donation-confirmation + admin-alert emails.
